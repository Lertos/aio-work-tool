...
python -m src
```
Keys 1-8 switch tabs. Window size, position and last tab are remembered.

## Data
Saved as JSON, one file per list, in `%APPDATA%\lertos\Work AIO Tool\`
(`todo.json`, `folders.json`, `copy.json`, `promote.json`, `info.json`, `sql_compare.json`, `surround.json`, `schema_backup.json`).
SQL passwords and Schema Backup connection strings are stored in Windows Credential Manager
...
src/
  ...
  services/  promoter.py (copy/move), sql_compare.py (definition compare), surround.py (prefix/suffix lines),
             odbc.py (picks the SQL Server driver),
             schema_backup.py (save definitions to .sql files)
  ui/        main_window.py, delegates.py (list rows), toast.py, worker.py, widgets.py
    tabs/    item_list_tab.py (shared base), todo, simple (folders/copy/info), promoter, sql_compare, surround, schema_backup
