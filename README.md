...
python -m src
```
Keys 1-6 switch tabs. Window size, position and last tab are remembered.

## Data
Saved as JSON, one file per list, in `%APPDATA%\lertos\Work AIO Tool\`
(`todo.json`, `folders.json`, `copy.json`, `promote.json`, `info.json`, `sql_compare.json`).
...
src/
  ...
  services/  promoter.py (copy/move), sql_compare.py (definition compare)
  ui/        main_window.py, delegates.py (list rows), toast.py, worker.py, widgets.py
    tabs/    item_list_tab.py (shared base), todo, simple (folders/copy/info), promoter, sql_compare
    dialogs/ item_form_dialog.py (shared base), simple, promote, sql_compare, sql_compare_run
```
